...
public class SimplifyDebts {
    ...
    while((edgePos = getNonVisitedEdge(solver.getEdges())) != null) {
      ...
      for(List<Dinics.Edge> allEdges : residualGraph) {
        for(Dinics.Edge edge : allEdges) {
          long capacity = ((edge.flow < 0) ? edge.capacity : (edge.capacity - edge.flow));
          if(capacity > 0) {
            newEdges.add(new Dinics.Edge(edge.from, edge.to, capacity));
          }
        }
      }
      ...
      System.out.println(String.format("%s ----%s---> %s", person[source], maxFlow, person[sink]));
      visitedEdges.add(getEdgeHashKey(source, sink));
  ...
  private static Integer getNonVisitedEdge(List<Dinics.Edge> edges) {
    ...
    for(Dinics.Edge edge : edges) {
      if(!visitedEdges.contains(getEdgeHashKey(edge.from, edge.to))) {
        edgePos = curEdge;
      }
      curEdge++;
    }
  ...
  private static Long getEdgeHashKey(int u, int v) {
    return u * OFFSET + v;
  }
}
...
abstract class NetworkFlowSolverBase {
  ...
  /**
   * Set 'solved' flag to false to force recomputation.
   */
  ...
  public void printEdges() {
    System.out.println("List of Edges in the flow network...");
    System.out.println("------------------------------------");
    System.out.println();
    for(Edge edge : edges) {
      System.out.println(String.format("%s-----%s----->%s", edge.from, edge.capacity, edge.to));
    }
    System.out.println();
  }
